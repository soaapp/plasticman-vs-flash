...
import { Zap, Atom, Trophy, Repeat, ChevronRight, Loader2, Activity, Sparkles, Swords } from "lucide-react";

/* ------------------------------------------------------------------ *
 *  PLASTIC MAN  vs  THE FLASH  —  Schrödinger's Showdown
 *  Hackathon lunch-time entertainment prototype
 *  - Animated character cards
 *  - Multi-agent fight (Flash agent + Plastic Man agent + Quantum Referee)
 *    running live on the Claude API (Claude-in-Claude)
 *  - Audience prediction poll
 *  - Simulated quantum collapse (swap in Qiskit Aer / real QPU in prod)
 * ------------------------------------------------------------------ */
...
.qs-vote { cursor:pointer; border:3px solid #000; border-radius:12px; padding:16px; text-align:center; font-family:'Bungee'; font-size:16px; transition:transform .1s, box-shadow .1s; background:var(--panel); }
...
export default function App() {
  ...
  async function runFight() {
    ...
    try {
      ...
      setError("The agents hit a snag reaching the Claude API. Try running the simulation again.");
    } finally {
      setFighting(false);
    }
  }

  // simulated quantum measurement (Hadamard -> measure). True circuit is ~50/50;
  // we let the narrative momentum nudge the amplitude slightly for fun, and keep
  // a real chance of the canonical STALEMATE.
  function collapse() {
    setCollapsing(true);
    ...
    setTimeout(() => {
      ...
      let result;
      if (roll < 0.22) result = "stalemate"; // the honest comic-book answer
      else result = Math.random() < pFlash ? "flash" : "plastic";
      setOutcome(result);
      setCollapsing(false);
      setScene("result");
    }, 2600);
  ...
  function reset() {
    setScene("intro"); setPrediction(null); setRounds([]); setOutcome(null); setError(null);
  }
  ...
  return (
    <div className="qs-root">
      ...
      <div className="qs-wrap">
        ...
        {scene === "quantum" && (
          <Quantum collapsing={collapsing} momentum={momentum} onMeasure={collapse} />
        )}

        {scene === "result" && (
          <Result outcome={outcome} prediction={prediction} onReplay={reset} />
        )}
      </div>
    </div>
  );
...
function Cards({ prediction, setPrediction, onNext }) {
  return (
    <div>
      ...
      <div className="qs-section">
        <div className="qs-kicker">Audience Poll</div>
        <h3 className="qs-h">WHO TAKES IT?</h3>
        <p className="qs-sub">Cast your prediction before the simulation runs. (Live 30-person voting lands in the production build with a backend — this prototype tracks your pick.)</p>
        <div className="qs-poll">
          <div className={`qs-vote plastic ${prediction === "plastic" ? "sel" : ""}`} onClick={() => setPrediction("plastic")}>PLASTIC MAN</div>
          <div className={`qs-vote flash ${prediction === "flash" ? "sel" : ""}`} onClick={() => setPrediction("flash")}>THE FLASH</div>
        </div>
      </div>
...
function Arena({ rounds, fighting, error, momentum, logRef, onFight, onCollapse }) {
  ...
  return (
    <div>
      <div className="qs-kicker">Multi-Agent Simulation · Live on Claude</div>
      <h2 className="qs-title" style={{ fontSize: 24 }}>THE BOUT</h2>
      <p className="qs-sub">A Flash agent and a Plastic Man agent each choose their moves in parallel; a Quantum Referee agent narrates and scores every round.</p>
...
function Quantum({ collapsing, momentum, onMeasure }) {
  const pFlash = 0.5 + Math.max(-0.18, Math.min(0.18, momentum * 0.05));
  return (
    <div className={`qs-quantum ${collapsing ? "qs-collapsing" : ""}`}>
      ...
      <div className="qs-note">
        SIMULATED MEASUREMENT. In the production build this calls a small backend running Qiskit:
        instant collapse on the Aer simulator for the live demo, with an optional pre-run on real
        real quantum hardware (job ID shown) for genuine quantum-decided bragging rights.
      </div>
    </div>
  );
}

function Result({ outcome, prediction, onReplay }) {
  const map = {
